...
async function processarDecisao(request: Request): Promise<Response> {
  ...
  if (error || !ferias) {
    return html("Solicitação não encontrada", "O link pode ter expirado ou já foi utilizado.", false);
  }

  if (ferias.status !== "Pendente") {
    return html("Já decidido", `Esta solicitação já está como <b>${ferias.status}</b>.`, false);
  }
...
// Endpoint público acessado a partir do link de aprovação enviado por e-mail/Teams.
export const Route = createFileRoute("/api/public/ferias/decisao")({
  server: {
    handlers: {
      GET: async ({ request }) => processarDecisao(request),
      POST: async ({ request }) => processarDecisao(request),
      HEAD: async () =>
